...
public interface CCMasterRepository extends JpaRepository<CCMaster, CCMasterKey> {
	...
	@Query(value = "select CAST(SUBSTRING(STMTMONTHYEAR, 1, 2) as UNSIGNED) as mm, SUBSTRING(STMTMONTHYEAR, 3, 6) as yyyy, SUM(CAST(TOTALAMT as UNSIGNED)) as amount from `freedb_cc-database`.CC_MASTER_TEST cmt where SUBSTRING(STMTMONTHYEAR, 3, 6) = ? and cmt.CODE <> 'PNB02' and cmt.CODE <> 'SIP01' and cmt.CODE <> 'SIP02' group by mm,yyyy order by mm", nativeQuery = true)
	List<AmountPerMonth> getAmountPerMonth(String year);
	
	@Query(value = "select CAST(SUBSTRING(STMTMONTHYEAR, 1, 2) as UNSIGNED) as mm, SUBSTRING(STMTMONTHYEAR, 3, 6) as yyyy, SUM(CAST(TOTALAMT as UNSIGNED)) as amount from `freedb_cc-database`.CC_MASTER_TEST cmt where SUBSTRING(STMTMONTHYEAR, 3, 6) = ? and cmt.CODE <> 'PNB02' and cmt.CODE <> 'SIP01' and cmt.CODE <> 'SIP02' and username = ? group by mm,yyyy order by mm", nativeQuery = true)
	...
	@Query(value = "select c2.name, SUM(CAST(c1.TOTALAMT as UNSIGNED)) as amount from `freedb_cc-database`.CC_MASTER_TEST c1, `freedb_cc-database`.CC_MASTER_NAMES c2 where c1.code = c2.code and substring(c1.STMTMONTHYEAR, 3, 7)= ? and c2.name <> 'PNB Car Loan' and c2.name <> 'PP SIP' and c2.name <> 'IC SIP' group by c2.name order by c2.name", nativeQuery = true)
	List<AmountPerMonth> getAmountPerCard(String year);
	
	@Query(value = "select c2.name, SUM(CAST(c1.TOTALAMT as UNSIGNED)) as amount from `freedb_cc-database`.CC_MASTER_TEST c1, `freedb_cc-database`.CC_MASTER_NAMES c2 where c1.code = c2.code and substring(c1.STMTMONTHYEAR, 3, 7)= ? and c2.name <> 'PNB Car Loan' and c2.name <> 'PP SIP' and c2.name <> 'IC SIP' and username = ? group by c2.name order by c2.name", nativeQuery = true)
